...
def sample_from_probs(probs, top_n=10):
    ...
    """
    if probs.ndim > 1:
        # If we have batch dimension, process each row separately
        if probs.ndim == 2:
            indices = []
            for i in range(probs.shape[0]):
                probs_i = probs[i].astype(np.float64)
                probs_i[np.argsort(probs_i)[:-top_n]] = 0
                probs_i /= np.sum(probs_i)
                indices.append(np.random.choice(len(probs_i), p=probs_i))
            return indices
        else:
